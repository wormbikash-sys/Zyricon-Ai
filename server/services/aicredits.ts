...
const MODEL_CATALOG_URL = 'https://api.aicredits.in/api/models';
...
export function getOpenAIClient(): OpenAI {
  const apiKey = process.env.AICREDITS_API_KEY || 'demo_key_nexus_ai';
  return new OpenAI({
...
export async function fetchAICreditsBalance(): Promise<{
  ...
  if (!apiKey) {
    return { balance: 100.0, currency: 'INR', status: 'Demo Mode (AICREDITS_API_KEY not configured in env)' };
  }
...
function simulateStreamingResponse({
  ...
  const mockResponses = [
    `Thank you for your prompt! Using model **${model}**, here is a comprehensive breakdown:

### Key Highlights
1. **High Performance**: Delivered via NexusAI backend architecture.
...
\`\`\`typescript
// NexusAI Service Example
import { NexusAIClient } from '@nexus/ai';

async function generateInsight(prompt: string) {
  const response = await NexusAIClient.stream({
    model: "${model}",
    prompt,
  });
  return response;
...
Is there anything specific you would like me to expand on or modify regarding "${userText.slice(0, 40)}..."?`,
  ];
